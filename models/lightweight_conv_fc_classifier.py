"""
LightweightConvFcClassifier - Enhanced ConvFcClassifier with Residual Blocks (no SE layers).
"""
import torch
...
class ResidualBlock(nn.Module):
    """
    A Residual Block as introduced in ResNet (without SE layer for lightweight version).
    """
...
class LightweightConvFcClassifier(nn.Module):
    def __init__(self, num_classes=3, target_length=32, cycle_num=2, lead_num=1):
        """
        Enhanced ConvFcClassifier with Residual Blocks (lightweight version without SE layers).
    ...
    def forward(self, x, modes=None):
        """
        Forward pass of the LightweightConvFcClassifier.
