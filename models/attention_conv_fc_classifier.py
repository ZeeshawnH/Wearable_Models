"""
AttentionConvFcClassifier - Enhanced ConvFcClassifier with Residual Blocks, SE layers, and Global Attention.
"""
import torch
...
class GlobalAttention(nn.Module):
    ...
    def forward(self, ecg_features, duration_features):
        ...
        """
        combined = torch.cat((ecg_features, duration_features), dim=-1)
        y = self.fc1(combined)
        y = self.elu(y)
        y = self.dropout(y)
        y = self.fc2(y)
...
class AttentionConvFcClassifier(nn.Module):
    def __init__(self, num_classes=3, target_length=32, cycle_num=2, lead_num=1):
        ...
        self.global_attention = GlobalAttention(
            ecg_feature_size=64*cycle_num*lead_num * target_length,
            duration_feature_size=64*cycle_num*lead_num,
            intermediate_size=128*cycle_num*lead_num
        )
        ...
        self.classifier = nn.Sequential(
            ...
            nn.ELU(inplace=True),
            nn.Dropout(0.5),
            nn.Linear(64*cycle_num*lead_num, 32*cycle_num*lead_num),
    ...
    def forward(self, x, modes=None):
        """
        Forward pass of the AttentionConvFcClassifier.
        ...
        attention_weights = self.global_attention(ecg_flat, duration_proj)

        attention_weights = attention_weights.view(ecg_features.size(0), 64*self.cycle_num*self.lead_num, -1)
        ecg_features = ecg_features * attention_weights
