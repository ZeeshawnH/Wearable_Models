...
def find_ecg_files(data_directory):
    header_files = list()
    recording_files = list()
    for f in sorted(os.listdir(data_directory)):
        root, extension = os.path.splitext(f)
        if not root.startswith('.') and extension == '.hea' and root.endswith('_ecg'):
            header_file = os.path.join(data_directory, f)
...
def find_all_challenge_files(main_directory):
    ...
    for subfolder in subfolders:
        header_files, recording_files = find_ecg_files(subfolder)
